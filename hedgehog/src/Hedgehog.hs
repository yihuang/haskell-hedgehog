...
import           Data.Functor.Classes           (Eq1, Ord1, Show1, compare1,
                                                 eq1, showsPrec1)
...
import           Hedgehog.Internal.Property     (eval, evalIO, evalM)
...
import           Hedgehog.Internal.Runner       (check, checkParallel,
                                                 checkSequential, recheck)
import           Hedgehog.Internal.Seed         (Seed (..))
import           Hedgehog.Internal.State        (Callback (..), Command (..))
import           Hedgehog.Internal.State        (Action, Parallel (..),
                                                 Sequential (..))
import           Hedgehog.Internal.State        (executeParallel,
                                                 executeSequential)
import           Hedgehog.Internal.State        (Concrete (..), Symbolic,
                                                 Var (..), concrete, opaque)
